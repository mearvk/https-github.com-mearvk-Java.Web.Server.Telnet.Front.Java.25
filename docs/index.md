...
<head>
  ...
  <style>
    :root{
      ...
      --sidebar-width: 310px; /* widened by 50px */
      --sidebar-bg: #fbfdff;
    ...
    .main {
      ...
      overflow: auto;
      max-height: 100vh;
      /* ensure content area has room for the custom slider on the right */
      padding-right: calc(28px + var(--slider-width) + 12px);
    }

    /* Make the content container span the full width of the main panel and center it */
    .container {
      width: 100%;
      max-width: none;
      margin: 0 auto;
    ...
    /* Make headings and paragraphs centered and allow them to use full width */
    .container h1,
    .container h2,
    .container h3,
    .container p,
    ...
    /* For readability, keep code blocks left-aligned inside a centered block */
    ...
    /* Vertical charcoal slider fixed on the right edge of the viewport */
    .charcoal-slider {
      position: fixed;
      right: 8px;
      top: 50%;
      transform: translateY(-50%) rotate(-90deg); /* rotate to make a vertical appearance */
      transform-origin: center;
      z-index: 9999;
      width: calc(100vh * 0.0); /* no layout width; input itself is rotated */
      display: flex;
      align-items: center;
    ...
    /* The visible wrapper to provide a vertical track */
    .charcoal-slider .track {
    ...
    /* The actual input range is rotated back to horizontal inside the wrapper */
    .charcoal-slider input[type="range"] {
      -webkit-appearance: none;
      appearance: none;
      width: 200px; /* length of the slider before rotation */
      height: 8px;
      background: transparent;
      transform: rotate(90deg); /* rotate to vertical orientation inside wrapper */
      outline: none;
      cursor: pointer;
    }

    /* Track (for WebKit) */
    .charcoal-slider input[type="range"]::-webkit-slider-runnable-track {
      height: 8px;
      background: var(--slider-track);
      border-radius: 6px;
    }
    /* Thumb (for WebKit) */
    .charcoal-slider input[type="range"]::-webkit-slider-thumb {
      -webkit-appearance: none;
      appearance: none;
      width: 18px;
      height: 18px;
    ...
    /* Firefox styling */
    .charcoal-slider input[type="range"]::-moz-range-track {
      height: 8px;
      background: var(--slider-track);
      border-radius: 6px;
    }
    .charcoal-slider input[type="range"]::-moz-range-thumb {
      width: 18px;
      height: 18px;
      background: var(--slider-thumb);
      border-radius: 50%;
    ...
    /* Small screens: move slider inside the main area to avoid overlap */
    @media (max-width: 920px) {
      .charcoal-slider { right: 12px; top: auto; bottom: 12px; transform: rotate(0deg); }
      .charcoal-slider .track { width: 160px; height: 36px; border-radius: 8px; padding: 4px; }
      .charcoal-slider input[type="range"] { transform: rotate(0deg); width: 160px; }
    }
...
<body>
  ...
  <!-- Charcoal slider UI on the right -->
  <div class="charcoal-slider" aria-hidden="false" title="Charcoal slider">
    <div class="track" role="presentation">
      <input id="charcoalRange" type="range" min="0" max="100" value="50" aria-label="Charcoal slider" />
    </div>
  </div>

  <script>
    // Optional: simple visual feedback — adjust main scrollbar thumb opacity based on slider value
    (function(){
      const slider = document.getElementById('charcoalRange');
      const root = document.documentElement;
      function apply(value){
        // map 0..100 to 0.3..1 opacity for the scrollbar thumb
        const opacity = 0.3 + (value / 100) * 0.7;
        // set a CSS variable used by the scrollbar thumb color (we'll blend with rgba)
        root.style.setProperty('--scrollbar-thumb', `rgba(51,51,51,${opacity})`);
      }
